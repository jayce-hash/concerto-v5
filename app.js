...
(() => {
  ...
  // UPDATED: align with one-page stacked layout CSS
  // - "loading" toggles the overlay
  // - "form" turns on body.show-form (kept on forever)
  // - "results" turns on body.has-plan (kept on forever)
  const show = (name)=>{
    ...
    if (name === 'form') {
      document.body.classList.add('show-form');   // reveals #screen-form
    }
    if (name === 'results') {
      document.body.classList.add('has-plan');    // reveals #screen-results
    }

    // Nice little fade/transition hook
    ...
    if (saved && !new URLSearchParams(location.search).get("a")) {
      ...
      resumeBtn.onclick = async () => {
        ...
        try { await generate(); }
          ...
          // In your one-page layout, this just hides the loader
          show('welcome');
        }
      };
    }
  ...
  function bindTmSearch(){
    ...
    async function run(){
      ...
      try{
        ...
        // attach click handlers to each "Use this event" button
        qsa('.suggest-item', list).forEach(item=>{
          item.querySelector('button')?.addEventListener('click', async (e)=>{
            ...
            try{
              ...
              // Keep user on Concert step; just move them down to hotel
              state.staying = true;
  ...
  async function generate(){
    ...
    try{
      ...
      // In one-page layout: keep welcome+form in place and reveal results
      show('results');
      savePlan();
    } catch (e) {
      console.error(e);
