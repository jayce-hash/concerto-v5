(() => {
  ...
  console.log("Concerto app.js v6.2.0 loaded");
  ...
  function buildNarrative(plan){
    ...
    if (state.eatWhen !== "after" && showM != null && beforePick){
      ...
      parts.push(`Plan to have dinner before at <strong>${esc(beforePick.name)}</strong> (~${beforePick.distance?.toFixed ? beforePick.distance.toFixed(1) : beforePick.distance} mi from the venue) around <strong>${format12(dinnerBeforeM)}</strong>.`);
    }
  ...
  function renderResults(plan){
    ...
    // Narrative (AI intro if present, else our builder)
    const narrative = plan.intro && plan.intro.trim() ? plan.intro : buildNarrative(plan);
    ...
    // Cards grid + narrative card at top
